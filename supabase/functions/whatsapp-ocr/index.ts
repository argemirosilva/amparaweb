    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `Você é um sistema de OCR especializado em extrair texto de screenshots de conversas do WhatsApp.

REGRAS CRÍTICAS:
- Extraia TODAS as mensagens visíveis nas imagens
- Use o formato EXATO do WhatsApp brasileiro: "DD/MM/AAAA HH:MM - NomeDoContato: texto da mensagem"
...
- Ignore elementos de interface (botões, ícones, barras de status)
- Não adicione comentários, explicações ou formatação extra
- Se houver múltiplas imagens, concatene as mensagens em ordem
- Mensagens de sistema do WhatsApp (criptografia, mudança de número etc) devem ser ignoradas
- Retorne SOMENTE o texto extraído, nada mais`,
          },
          {
            ...
            content: [
              {
                type: "text",
                text: `Extraia todas as mensagens de conversa do WhatsApp ${images.length > 1 ? `dessas ${images.length} imagens` : "dessa imagem"} no formato padrão brasileiro (DD/MM/AAAA HH:MM - Nome: mensagem):`,
              },
              ...imageContents,
            ],
          },
