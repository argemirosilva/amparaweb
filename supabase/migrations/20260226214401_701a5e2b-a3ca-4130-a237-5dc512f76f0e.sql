...
CREATE OR REPLACE FUNCTION public.search_agressor_candidates(
  ...
  p_forca_seguranca boolean DEFAULT NULL,
  p_tem_arma boolean DEFAULT NULL,
  p_cor_raca text DEFAULT NULL,
  p_escolaridade text DEFAULT NULL,
  p_company text DEFAULT NULL,
  p_xingamentos text DEFAULT NULL
)
RETURNS TABLE(
...
DECLARE
  ...
  -- Count how many criteria were provided
...
BEGIN
  ...
  FROM agressores a
  WHERE (
    -- Each criterion must match with HIGHER thresholds
  ...
  -- SCORING: require at least 2 matching criteria when multiple were provided
  AND (
    criteria_count <= 1
    OR (
  ...
  -- Exclude weak-only matches: boolean fields alone shouldn't qualify
  AND NOT (
    criteria_count >= 2
